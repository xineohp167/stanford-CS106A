...
public class FlyTunesStore extends ConsoleProgram {
	...
	/**
	 * Prompts the user to pick a selection from a menu of options. Returns the
	 * users selection. Note that there is no bounds checking done on the users
	 * selection.
	 */
	...
	/**
	 * Checks to see if the song (defined by its name and the band that performs
	 * it) is already in the store. It returns the index of the song in the
	 * store's song list if it already exists and -1 otherwise.
	 */
	...
	/**
	 * Adds a new song to the store's inventory and returns that song to the
	 * caller. If the song already exists in the store, it returns the existing
	 * song from the inventory. Otherwise it returns the new song that was just
	 * added to the inventory. The method may return null if the user decides
	 * not to enter a song (i.e., user just presses Enter when asked for the
	 * song name).
	 */
	...
	/**
	 * Adds a new album to the store's inventory. If the album already exists in
	 * the store, then the inventory is unchanged. Otherwise a new album and any
	 * new songs it contains are added to the store's inventory.
	 */
	...
	/**
	 * Updates the price of a song in the store's inventory. Note that this
	 * price update will also affect all albums that contain this song.
	 */
